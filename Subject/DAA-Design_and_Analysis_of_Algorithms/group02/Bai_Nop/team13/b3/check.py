...
def main():
    ...
    for inp in sorted(tests):
        ...
        checker_name = "checker.exe" if platform.system() == "Windows" else "checker"
        if os.path.exists(checker_name):
            cmd = [checker_name, inp, out, ans]
            ...
            with open(out) as f1, open(ans) as f2:
                if f1.read().strip() == f2.read().strip():
                    print(f"✅ K | time={t:.2f}s | mem={mem:.1f}MB")
                    passed += 1
                else:
                    print(f"❌ A | time={t:.2f}s | mem={mem:.1f}MB")
                    failed += 1
