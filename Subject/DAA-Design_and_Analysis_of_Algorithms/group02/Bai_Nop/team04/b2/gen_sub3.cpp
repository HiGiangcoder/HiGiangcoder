...
void genNormal() {
    int n = rnd.next(50000, MAX_N);
    int mTarget = min(MAX_M, rnd.next(3 * n, 5 * n));
    ...
    int fail = 0;
    while ((int)edges.size() < mTarget && fail < 2000) {
        int u = rnd.next(1, n);
        int v = rnd.next(1, n);
        long long w = rnd.next(1, 1000000);
...
void genSpecialTrap() {
    ...
    int ladderLen = min(n - 1, 2000);
    for (int i = 1; i <= ladderLen; ++i) {
        int u = i;
        int v = i + 1;
        addEdge(used, edges, u, v, rnd.next(1, 10));
        if (i + 2 <= ladderLen + 1) {
            addEdge(used, edges, u, i + 2, rnd.next(20, 100));
        }
    }

    for (int i = 0; i < 5000 && (int)edges.size() < MAX_M; ++i) {
        int u = rnd.next(1, n);
        int v = rnd.next(1, n);
...
void genSpecialDisconnected() {
    ...
    int extra = min(MAX_M - (int)edges.size(), rnd.next(0, 5000));
    int fail = 0;
    while (extra-- > 0 && fail < 2000) {
        int u, v;
        ...
        long long w = rnd.next(1LL, 1000000000LL);
        if (addEdge(used, edges, u, v, w)) fail = 0;
        else ++fail;
    }
    printGraph(n, edges);
...
void genSpecialBfsTrap() {
    ...
    set<pair<int, int>> used;
    vector<Edge> edges;
    edges.reserve(min(6LL * n, (long long)MAX_M));

    for (int i = 1; i < n && (int)edges.size() < MAX_M; ++i)
        addEdge(used, edges, i, i + 1, 1000000000LL - i);

    for (int i = 3; i <= n && (int)edges.size() < MAX_M; ++i) {
        int v = rnd.next(1, i - 1);
        addEdge(used, edges, i, v, 1);
    }

    for (int i = 1; i + 5 <= n && (int)edges.size() < MAX_M; i += 5)
        addEdge(used, edges, i, i + 5, 1);

    printGraph(n, edges);
}

void genSpecialDuplicateEdges() {
    int n = rnd.next(40000, 65000);
    ...
    for (size_t i = 0; i < baseEdges.size(); ++i) {
        ...
        weights[0] = rnd.next(1, 1000000);
        for (int j = 1; j < copies; ++j) {
            weights[j] = weights[j - 1] + rnd.next(1, 1000000);
        }
...
void genRealBfsKiller() {
    ...
    for (int i = 1; i < n && (int)edges.size() < MAX_M; ++i)
        addEdge(used, edges, i, i + 1, 1);

    for (int i = 1; i + 3 <= n && (int)edges.size() < MAX_M; ++i)
        addEdge(used, edges, i, i + 3, 100'000);
...
void genSpecial(int variant) {
    if (variant == 2) genSpecialHeavy();
        ...
        for (int i = 1; i < n && (int)edges.size() < MAX_M; ++i) addEdge(used, edges, i, i + 1, 1000000000LL - i);
        for (int i = 1; i + 7 <= n && (int)edges.size() < MAX_M; ++i) addEdge(used, edges, i, i + 7, 5);
        for (int i = 2; i <= n && (int)edges.size() < MAX_M; ++i) addEdge(used, edges, i, i - 1, 500000000LL);
        printGraph(n, edges);
    } else if (variant == 5) {
        int n = rnd.next(50000, 80000);
        set<pair<int, int>> used;
        vector<Edge> edges;
        edges.reserve(3 * n);
        for (int i = 1; i < n && (int)edges.size() < MAX_M; ++i)
            addEdge(used, edges, i, i + 1, 1000000000LL - i);
        for (int i = 1; i + 3 <= n && (int)edges.size() < MAX_M; ++i)
            addEdge(used, edges, i, i + 3, 1);
        for (int i = 1; i + 8 <= n && (int)edges.size() < MAX_M; ++i)
            addEdge(used, edges, i, i + 8, 2);
        for (int i = 2; i <= n && (int)edges.size() < MAX_M; ++i)
            addEdge(used, edges, i, i - 1, 600000000LL);
        printGraph(n, edges);
    } else if (variant == 6) genSpecialBfsTrap();
    else if (variant == 7) genRealBfsKiller();
    else if (variant == 8) genSpecialDuplicateEdges();
    else genSpecialTrap();
}
...
void genEdge() {
    ...
    int n = MAX_N;
    int mTarget = min(100, rnd.next(n - 1, MAX_M));
    edges.reserve(mTarget);
    vector<int> perm(n);
    iota(perm.begin(), perm.end(), 1);
    ensurePath(used, edges, perm, 1, 1);
    int fail = 0;
    while ((int)edges.size() < mTarget && fail < 2000) {
        int u = rnd.next(1, n);
        int v = rnd.next(1, n);
