...
$to_build = @(
    ...
    @{src="gen_sub3.cpp"; out="gen_sub3.exe"}
)
...
function Gen-And-Run($gen_bin, $type, $start, $end) {
    for ($i = $start; $i -le $end; $i++) {
        ...
        # Sinh input
        & "./$gen_bin.exe" $type $seed | Set-Content -Encoding ASCII $input_file

        # Kiểm tra input
        $valid = Get-Content -Encoding ASCII $input_file | & ./validator.exe
...
Gen-And-Run "gen_sub1" 1 1 30

Write-Host "[Generate] Subtask 2"
Gen-And-Run "gen_sub2" 2 31 60

Write-Host "[Generate] Subtask 3"
Gen-And-Run "gen_sub3" 2 61 100
