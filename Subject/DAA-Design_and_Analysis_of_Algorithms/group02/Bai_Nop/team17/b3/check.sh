...
if [[ "$SRC" == *.cpp ]]; then
  ...
  # SỬA LẠI: Dùng đúng tên g++ và cờ rpath
  if ! g++-15 -std=c++17 -O2 -pipe -s -Wl,-rpath,/opt/homebrew/lib "$SRC" -o "$EXEC"; then
    echo "❌ Compile failed"
    exit 1
  fi
else
...
# SỬA LẠI: Dùng đúng tên g++ và cờ rpath cho validator/checker
[ -f validator.cpp ] && g++-15 -std=c++17 -O2 -pipe -s -Wl,-rpath,/opt/homebrew/lib validator.cpp -o validator 2>/dev/null || true
[ -f checker.cpp ]   && g++-15 -std=c++17 -O2 -pipe -s -Wl,-rpath,/opt/homebrew/lib checker.cpp   -o checker   2>/dev/null || true
...
for d in "${tests[@]}"; do
  ...
  # --- chạy solution ---
  # SỬA LẠI: Đổi timeout -> gtimeout VÀ /usr/bin/time -> gtime
