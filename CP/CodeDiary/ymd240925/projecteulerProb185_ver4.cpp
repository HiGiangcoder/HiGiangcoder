...
struct Seed {
    ...
    bool Calculate() {
        ...
        for (int i = 1; i <= NUM; i ++) {
            int cnt(0);
            for (int j = 0; j < LEN; j ++) cnt += str[j] == s[i][j];
...
int main() {
    ...
    for (int i = 0; i < LEN; i ++) {
        for (int j = 0; j < 10; j ++) {
            for (int k = 0; k < 10; k ++) {
                string s = "";
                for (int x = 0; x < LEN; x ++) s += char(random(0, 9) + '0');
                s[i] = char(j + '0');
                a[i][j][k].str = s;
                a[i][j][k].Calculate();
            }
    ...
    while (true) {
        ...
        for (int i = 0; i < LEN; i ++) for (int j = 0; j < 10; j ++) {
        ...
        for (int i = 0; i < LEN; i ++) for (int i2 = 0; i2 < 10; i2 ++) {
            for (int k = 0; k < 10; k ++) {
                b[m ++] = a[i][i2][k];
                for (int j = 0; j < LEN; j ++) {
                    for (char c = '0'; c <= '9'; c ++) if (c != a[i][i2][k].str[j]){
                        ...
                        while (random(0, random(0, LEN)) != 0) {
                            b[m] = b[m - 1];
                            b[m].str[random(0, LEN - 1)] = char(random(0, 9) + '0');
                            if (b[m].Calculate())m ++;
                        }
                    }
                }
